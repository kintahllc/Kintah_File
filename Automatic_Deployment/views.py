...
def create_postgresql_db(db_name, db_user, db_password):
    ...
    try:
        ...
        if not user_exists:
            # Create a new user for the Odoo instance
...
def create_instance_view(request, company_info_id, setup_id):
    ...
    try:
        ...
        instance_name = f"Odoo-{domain_name}-instance"
    ...
    try:
        print("Trying to Attach static IP with Instance ...")
        # Attach static IP
        client.attach_static_ip(
            staticIpName=setup.static_ip_name,
            instanceName=setup.instance_name
        )
        print("Successfully Attached static IP with Instance !")
    except Exception as e:
